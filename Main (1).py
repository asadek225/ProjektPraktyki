...
def gate_cost_from_tuple(t):
    # 1- i 2-argumentowe koszt 1, większe koszt 5
...
def run_all(
    ...
    progress_every=1000,
    print_gates=False,          # ustaw True jeśli chcesz wypisywać bramki
    print_first_n=3             # ile pierwszych permutacji wypisać
):
    ...
    with open_out(output_path, "wt", encoding="utf-8") as out_f:
        for idx, vectors in enumerate(iter_jsonl(input_path), start=1):
            total = idx
            f = sim.TruthTable(3).set_vectors(vectors)
            cir = sim.Circuit()
            ...
            if suppress_output:
                ...
                al.algorithm(f, cir, verbose=True)

            instr = list(cir.instructions)                    # [(target, *controls), ...]
            instr_names = [gate_label(t) for t in instr]      # ["QNOT", "CNOT", ...]
            num_gates = len(instr)
            ...
            record = {
                ...
                "gates_used": dict(Counter(instr_names)),      # podsumowanie: ile których bramek
                "instructions": [
    ...
    # sortowanie histogramów po kluczu (rosnąco) + zapis statystyk
...
def main():
    run_all(
        input_path="permutacje_n3.jsonl",  # lub .gz
        output_path="results_n3.jsonl",
        stats_path="stats_n3.json",
        suppress_output=True,              # wycisza printy z algorithm
        progress_every=1000,
        print_gates=True,                  # <— WŁĄCZ
        print_first_n=3                    # <— ile pierwszych permutacji wypisać
    )
